...
旧取得スクリプトの日時欠落は、DX NETで同じ譜面・スコア・プレイヤーの日時を確認してから `repairMaimaiMissingDate_(sheet, row, originalSha256, verifiedRow)` で単独投稿を再検証できます。原本・ハッシュを保持し、補正履歴を残します。通常受付での日時自動補完はしません。
